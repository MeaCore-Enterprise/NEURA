...
export class PlayerController {
    constructor(engine) {
        this.store = createStore(initialState, playerReducer);
        this.engine = engine;
    ...
    async playIndex(index) {
        ...
        if (index < 0 || index >= s.tracklist.length)
            return;
    ...
    onEnded() {
        ...
        if (nextIndex !== s.currentIndex)
            this.playIndex(nextIndex);
    }
    chooseNextIndex(s, currentIndex) {
        if (s.mode === "FOCUS")
            return currentIndex;
        if (s.mode === "CHILL") {
            if (s.tracklist.length <= 1)
                return currentIndex;
