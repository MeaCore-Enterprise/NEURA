...
export function playerReducer(state: PlayerCoreState, action: PlayerAction): PlayerCoreState {
  switch (action.type) {
    case "SET_MODE": {
    ...
    case "PLAY_REQUEST": {
      if (action.index < 0 || action.index >= state.tracklist.length) return state;
